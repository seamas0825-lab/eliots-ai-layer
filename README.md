...
部分浏览器、Electron 应用和微信不公开标准的辅助功能选中文本。应用会在拖动或双击划词后临时执行一次复制、读取文字并立即恢复原剪贴板，因此可以兼容 Safari、Ego、飞书、ChatGPT 和微信等应用，也不会覆盖原有剪贴板或把内部取词写入剪贴板历史。
...
解释、总结和“问 AI”会输出原生渲染的 Markdown。配置 OpenAI 官方 API 时使用 Responses API 的 `web_search`，配置阿里云百炼时使用 `enable_search`，Perplexity 模型使用其原生联网能力。若当前 API（例如 DeepSeek 官方 Chat Completions）没有提供可调用的原生搜索，结果会明确标记“未联网”，不会伪装成实时答案。
